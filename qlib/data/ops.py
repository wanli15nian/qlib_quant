...
class CutMoM(PairRolling):
    ...
    def __init__(self, feature_left, feature_right, N):
        super(CutMoM, self).__init__(feature_left, feature_right, N, "cutmom")
    ...
    def _load_internal(self, instrument, start_index, end_index, *args):
        ...
        def func(a, b):
            thres_n = heapq.nsmallest(int(self.N * 0.7), b)[-1]
            return np.sum(a[b <= thres_n])

        try:
            series = pd.Series(rolling_apply(func, self.N, series_left, series_right), index=series_left.index)
